...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_files, split="all"):
        ...
        self.dataset = load_dataset(path=DATASET_SCRIPT_PATH, data_files=data_files, split=split)
...
def truncate_fn(
    dataset: Dataset, tokenizer, max_seq_length: int = 128, fill_to_max: bool = False
) -> Dataset:
    ...
    while i < len(dataset["tokens"]):
        ...
        if current_tokens_len > max_seq_length - 2:
            ...
            if fill_to_max and (i + 1 < len(dataset["tokens"])):
                ...
                if (
                    current_tokens_len + next_tokens_len + 1 < max_seq_length - 2
                    and dataset["file_id"][i] == dataset["file_id"][i + 1]
                ):
