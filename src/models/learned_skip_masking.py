...
class DeepSkipAugmenter(torch.nn.Module):
    ...
    def forward(
        ...
        masker_output = self.masker(input_ids = input_ids, return_hidden=False) # [Batch, SeqLen, 1]

        # Apply sigmoid to masker_output to get masking probabilities
        mask_probas = torch.sigmoid(masker_output).squeeze(dim=-1) * maskable_tokens # [Batch, SeqLen]
        # If probability of masking > 50%, replace with [MASK] token
        tokens_to_mask = mask_probas > 0.5 # [Batch, SeqLen]
        # Mask token id [MASK] from BERT
        ...
        if return_mask:
            return masker_output, cls_out, tokens_to_mask
        else:
            return masker_output, cls_out
...
class DeepSkipAugmenterTrainer:
    def __init__(
        self,
    ...
    def train_one_epoch(self, epoch, model, optimizer, criterion, dataloader, logger):
        ...
        for idx, batch in enumerate(dataloader):
            ...
            optimizer.zero_grad()
            masker_output, cls_out, tokens_to_mask = model(
                input_ids=input_ids,
                attention_mask=attention_mask,
